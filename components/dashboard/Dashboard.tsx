...
const Dashboard: React.FC<DashboardProps> = ({ transactions, currency, onQuickLog, onAddTransaction }) => {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    const localStartOfMonth = new Date(startOfMonth.getTime() - (startOfMonth.getTimezoneOffset() * 60000)).toISOString().split('T')[0];
    ...
    <div className="space-y-6 py-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          <StatCard title="Net Balance" amount={balance} currency={currency} type="balance" />
        </div>
        <StatCard title="Total Income" amount={totalIncome} currency={currency} type="income" />
        <StatCard title="Total Expense" amount={totalExpense} currency={currency} type="expense" />
      </div>

      <QuickLogButtons onQuickLog={onQuickLog} onAddTransaction={onAddTransaction} />
      ...
      {transactions.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center text-gray-500 pt-16">
          ...
          <p className="mt-2">Log your first transaction to see your financial overview.</p>
        </div>
      ) : filteredTransactions.length === 0 ? (
         <div className="flex flex-col items-center justify-center text-center text-gray-500 pt-16">
          <Icon name="empty" className="w-24 h-24 mb-4" />
          <h2 className="text-xl font-semibold text-gray-300">No Transactions Found</h2>
          <p className="mt-2">There are no transactions for the selected time period.</p>
        </div>
      ) : (
        <>
          ...
          <div>
            <RecentTransactions transactions={filteredTransactions.slice(0, 5)} currency={currency} />
          </div>
        </>
      )}
    </div>
